...
# -*- coding: utf-8 -*-
"""
Created on Fri Aug 25 22:29:44 2023
...
def generate_brightness_variation_and_compressed_images(original_images, labels, variation_intensity, compression_ratio):
    # Generate brightness variation images
    ...
    decompressed_images, _, file_sizes = generate_compressed_images_jpeg2000(brightness_variation_images, labels, compression_ratio)

    return original_images, brightness_variation_images, labels, decompressed_images, file_sizes



def generate_gradual_brightness_variation(data_dir, categories, variation_intensity, compression_ratio, num_iterations=4):
    # Load an example image
    image_path = os.path.join(data_dir, categories[1], os.listdir(os.path.join(data_dir, categories[1]))[1])
    ...
    for i in range(num_iterations):
        ...
        # Compress and decompress the brightened image
        compressed_image, _, _ = generate_compressed_images_jpeg2000([brightened_image], [categories[0]], compression_ratio)
...
def plot_gradual_brightness_variation(images, noise_samples, decompressed_images):
...
def image_preprocessing_with_compression(categories, clean_images, brightness_variation_images, labels, decompressed_images, file_sizes):

    # Calculate and print the average PSNR for noisy images
    avg_psnr_noisy = calculate_average_psnr(clean_images, brightness_variation_images)
...
#%%
def evaluate_models(data_dir, variation_intensity, compression_ratio, num_iterations, model_names):
    ...
    for iteration in range(num_iterations):
        ...
        # Generate brightness variation and compressed images
        original_images, brightness_variation_images, labels, decompressed_images, file_sizes = generate_brightness_variation_and_compressed_images(original_images, labels, variation_intensity, compression_ratio)
        train_images, val_images, test_images, train_labels, val_labels, num_classes, class_names, avg_psnr_noisy, avg_psnr, avg_size, avg_bpp = image_preprocessing_with_compression(categories, clean_images, brightness_variation_images, labels, decompressed_images, file_sizes)
        ...
        for model_name in model_names:
            ...
            avg_shift_noisy = average_shift(model, clean_images, brightness_variation_images)
            ...
            most_shifted_blurs = [brightness_variation_images[i] for i in most_shifted_indices]
            ...
            least_shifted_blurs = [brightness_variation_images[i] for i in least_shifted_indices]
            ...
            closest_blurs = [brightness_variation_images[i] for i in closest_indices]
...
#%%
def run_experiment_four_brightness(data_dir):
    # Define the categories and model names
    categories = os.listdir(data_dir)
    categories = categories[:20]
    model_names = ['VGG16', 'MobileNet', 'DenseNet121']

    # Generate gradual brightness variation images, noise samples, and decompressed images
    images, noise_samples, decompressed_images = generate_gradual_brightness_variation(data_dir, categories, variation_intensity = 1.5, compression_ratio = 30, num_iterations = 4)
    plot_gradual_brightness_variation(images, noise_samples, decompressed_images)

    # Call the evaluate_models function
    metrics35 = evaluate_models(data_dir, variation_intensity = 1.5, compression_ratio = 30, num_iterations = 10, model_names = model_names)
    metrics36 = evaluate_models(data_dir, variation_intensity = 1.5, compression_ratio = 60, num_iterations = 10, model_names = model_names)
    metrics37 = evaluate_models(data_dir, variation_intensity = 1.5, compression_ratio = 90, num_iterations = 10, model_names = model_names)

    num_iterations = 10
    iterations = [str(i) for i in range(1, num_iterations + 1)]
    for model_name in model_names:
        print(f'Generating plots for {model_name}...')
        generate_plots_for_model(model_name, metrics35, metrics36, metrics37, iterations)
        print(f'Plots for {model_name} generated successfully.\n')

    # Plotting heatmaps
    metrics_list = [metrics35, metrics36, metrics37]
