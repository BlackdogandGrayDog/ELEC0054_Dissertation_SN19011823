...
# -*- coding: utf-8 -*-
"""
Created on Sat Aug 26 00:19:33 2023
...
def generate_noisy_images_with_color_variation(data_dir, categories, salt_prob, pepper_prob, variation_level):
    ...
    noisy_images, noise_patterns, _ = generate_noisy_images(color_varied_images, labels, salt_prob, pepper_prob)
    ...
    return original_images, color_varied_images, noisy_images, noise_patterns, labels, total_noise



def plot_distortions(clean_image, salt_prob, pepper_prob):
    ...
    for i in range(0, 5):
        ...
        noisy_image_array, _ = generate_salt_and_pepper_noise(distorted_image_array, salt_prob, pepper_prob)
...
def image_preprocessing(categories, original_images, color_varied_images, noisy_images, labels, salt_prob, pepper_prob):
    # Plot the first original, color-varied, and noisy images for example
    plot_distortions(original_images[180], salt_prob, pepper_prob)
...
def visualize_decision_boundary_svm_shifted_topn(categories, model, images, labels, class_names, original_points, noisy_points, compressed_points, n_pairs=5, resolution=500, padding=12.0):
    ...
    for n in [1, n_pairs]:
        ...
        for i, (original_feature_2d, noisy_feature_2d, compressed_feature_2d) in enumerate(zip(original_features_2d, noisy_features_2d, compressed_features_2d)):
          ...
          legend_elements.append(Line2D([0], [0], linestyle="none", marker='o', markersize=10, markerfacecolor=compressed_color, label=f'Salt and Pepper Noisy - {compressed_class}'))
...
def visualize_decision_boundary_svm_top10_closest_and_shifted(categories, model, images, labels_int, class_names, original_points, noisy_points, compressed_points, resolution=500, padding=7.0):
    ...
    for i in range(10):
        ...
        legend_elements.append(Line2D([0], [0], linestyle="none", marker='o', markersize=10, markerfacecolor=compressed_colors[i], label=f'Salt and Pepper Noisy - {compressed_class} - Pair {i+1}'))
        legend_elements.append(Line2D([0], [0], color=arrow_colors[i], lw=2, label=f'Arrow 1 - Pair {i+1}'))
        legend_elements.append(Line2D([0], [0], color=arrow_colors[i+1], lw=2, label=f'Arrow 2 - Pair {i+1}'))
...
def evaluate_models(data_dir, salt_probs, pepper_probs, variation_level, model_names):
    ...
    for salt_prob, pepper_prob in zip(salt_probs, pepper_probs):
        ...
        original_images, color_varied_images, noisy_images, _, labels, _ = generate_noisy_images_with_color_variation(data_dir, categories, salt_prob, pepper_prob, variation_level)
        train_images, val_images, test_images, train_labels, val_labels, num_classes, class_names, avg_psnr_noisy, avg_psnr_colour = image_preprocessing(categories, original_images, color_varied_images, noisy_images, labels, salt_prob, pepper_prob)
        ...
        for model_name in model_names:
            ...
            if avg_psnr_colour == float('inf'):
                avg_psnr_colour = 100
            metrics[model_name][(salt_prob, pepper_prob)]['avg_psnr_colour_list'].append(avg_psnr_colour)
            ...
            metrics[model_name][(salt_prob, pepper_prob)]['avg_colour_shift_list'].append(avg_shift_variation)
...
def generate_plots_for_model(model_name, metrics1, metrics2, metrics3, metrics4, metrics5, probs):
    metric_pairs = [
        ('probs', 'avg_psnr_noisy_list', 'Probability of Salt & Pepper Noise', 'Average PSNR (Noise)'),
        ('probs', 'avg_psnr_colour_list', 'Probability of Salt & Pepper Noise', 'Average PSNR (Colour)'),
        ...
        ('probs', 'avg_colour_shift_list', 'Probability of Salt & Pepper Noise', 'Average Shift (Colour)'),
...
def plot_heatmap(metrics_list, model_names, probs, metric_key, title):
    ...
    for model_name in model_names:
        ...
        # Iterate through the colour variation levels
        for metric in metrics_list:
            row = []
            for prob in probs:
                # Extract the value for this probability and colour variation level
                value = metric[model_name][(prob, prob)][metric_key][0] # Adjust as needed
                row.append(value)
            values.append(row)

        # Convert the 2D array to a Pandas DataFrame
        df = pd.DataFrame(values, index=colour_variation, columns=probs)
...
def sensitivity_analysis(metrics_list, model_names, probs, metric_key, title):
    ...
    for model_name in model_names:
        for idx, metric in enumerate(metrics_list):
            ...
            plt.plot(probs, y_values, label=f'{model_name} (Colour Variation Level = {colour_variation[idx]})', color=colors[color_idx], linewidth=4)
            color_idx += 1
...
def plot_3d_surface(metrics_list, model_names, probs, metric_key, title):
    ...
    for model_name in model_names:
        ...
        X = np.array(X).reshape(len(colour_variation), len(probs))
        Y = np.array(Y).reshape(len(colour_variation), len(probs))
        Z = np.array(Z).reshape(len(colour_variation), len(probs))
...
def run_experiment_five_colour_salt(data_dir):
    """
    Run an experiment involving colour variations and salt and pepper noise.

    The function:
    1. Sets up directory, categories, model names, and probabilities.
    2. Evaluates the models under different variations of salt and pepper noise.
    ...
    # 2. Evaluate models
    metrics48 = evaluate_models(data_dir, salt_probs, pepper_probs, variation_level=0, model_names=model_names)
    metrics49 = evaluate_models(data_dir, salt_probs, pepper_probs, variation_level=3, model_names=model_names)
    metrics50 = evaluate_models(data_dir, salt_probs, pepper_probs, variation_level=6, model_names=model_names)
    metrics51 = evaluate_models(data_dir, salt_probs, pepper_probs, variation_level=8, model_names=model_names)
    metrics52 = evaluate_models(data_dir, salt_probs, pepper_probs, variation_level=9, model_names=model_names)
    metrics_list = [metrics48, metrics49, metrics50, metrics51, metrics52]

    # 3. Generate plots
    for model_name in model_names:
        print(f'Generating plots for {model_name}...')
        generate_plots_for_model(model_name, metrics48, metrics49, metrics50, metrics51, metrics52, probs)
